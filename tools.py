        ...
        for episode in json_data["episodes"]:
            for scenes in episode["scenes"]:
                ...
                for uttr in scenes["utterances"]:
                    us = tokenizer.encode(uttr["transcript"])[:-1]
...
def new_load_data(file_name, tokenizer, label_seq):
    ...
    with open(file_name, 'r', encoding='utf8')as fp:
        ...
        for scene in json_data:
            ...
            sep_place = [0]
            for uttr in scene:
                us = tokenizer.encode(uttr["utterance"])[:-1]
                if len(tokens) + len(us) + 1 < 512:
                    tokens += us
                    tokens.append(102)
                    k = uttr["emotion"]
        ...
        for dialogue in dialogues:
            ...
            for uttr in dialogue:
                if len(uttr["utterance"]) == 0:
                    continue
                us = tokenizer.encode(uttr["utterance"])[:-1]
                us_de = tokenizer.encode(uttr["utterance_de"])[:-1]
                us_fr = tokenizer.encode(uttr["utterance_fr"])[:-1]
                us_it = tokenizer.encode(uttr["utterance_it"])[:-1]
                if len(tokens) + len(us) + 1 < 512:
                    tokens += us
                    tokens.append(102)
                    k = uttr["emotion"]
...
def load_label_data(context_file, emotion_file, tokenizer, label_seq):
    ...
    for s_line, e_line in zip(cf, ef):
        ...
        for i, sentence in enumerate(sentences):
            us = tokenizer.encode(sentence)[: -1]
            if len(tokens) + len(us) + 1 < 512:
                tokens += us
                tokens.append(102)
                k = emotions[i]
